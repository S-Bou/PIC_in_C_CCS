...
////  After the user types in the sentence, the words are searched   ////
...
////           2              C5                                     ////
////           3              C4                                     ////
////           4              GND with 0.22uF cap to ISD pin 27      ////
////         5-10             NC                                     ////
////          11              GND                                    ////
////          12              GND                                    ////
...
////           2              GND (Prototype card)                   ////
////           3              Wiper of external pot                  ////
////           4              GND (Prototype card)                   ////
////           5              Positive lead of speaker               ////
////           6              +5V (Prototype card)                   ////
////           7              GND (Prototype card) with 0.05uF cap   ////
...
void get_address_list(char *s, long *l) { // parses the sentence and gets the address list
   int i,j,w_begin,count;
   char word[WORD_LENGTH];

   w_begin=0;
   count=0;
   for(i=0;i<SENTENCE_LENGTH;i++)         // go through all letters of the sentence
   {
      if((s[i]==' ')||(s[i]==0))          // go until the end of the word is found
      {
         ...
         if(s[i]==0)                      // quit when end of sentence
            break;
...
void main()   {
   ...
   while(TRUE)
   {
      ...
      printf("\r\nEnter a sentence for me to speak.  Push ENTER when done.\r\n");
      printf("*NOTE*  Please do not use any punctuation.\r\n");
      get_string(sentence,SENTENCE_LENGTH);     // gets the string

      get_address_list(sentence,address_list);  // gets the address list

      delay_ms(500);

      printf("\r\nNow speaking sentence:\n");
      i=0;
