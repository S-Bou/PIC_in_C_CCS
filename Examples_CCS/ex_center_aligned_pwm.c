...
////  with Dedicated Timers Module in Center-Aligned PWM mode. The   ////
...
void main(void) 
{
  ...
  while(TRUE) 
  {
    ...
    /* Set the duty cycle using the adc reading.  The second parameter of set_compare_time()
    sets the time that OC1 will rise, and the third parameters sets the time that OC1 will
    fall.*/
    set_compare_time(1, 32000 - (((unsigned int32)32000*duty)/100), 32000 + (((unsigned int32)32000*duty)/100));         
